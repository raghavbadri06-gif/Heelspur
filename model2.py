...
RESULTS = r"/nfsshare/users/raghavan/heelspurfinal/dinoresults/"
...
# -----------------------------
# 4) Novel Enhancement: DINOv2 ViT with Adapted CBAM
# 
# Key Changes:
# - Backbone: DINOv2 ViT for self-supervised features excelling in medical transfer learning.
# - Adapted CBAM: Directly applies to the spatial feature map (B, C, 14, 14) from TIMM's features_only.
# - No need for sequence-to-spatial reshape, as TIMM provides spatial format for ViT.
# - Hierarchical: Spatial/channel on maps -> global on pooled features.
# - Easy swap: Change backbone_name for other models.
# -----------------------------
...
class DINOv2AttentionModel(nn.Module):
    def __init__(self, backbone_name='vit_base_patch16_224.dino', pretrained=True, num_classes=3):
        super().__init__()
        # Backbone configured for feature extraction
        self.backbone = timm.create_model(backbone_name, pretrained=pretrained, num_classes=0, features_only=True)
        
        # Get feature dimension from last stage (C from (B, C, H, W))
        dummy_input = torch.zeros(1, 3, IMAGE_SIZE, IMAGE_SIZE)
        with torch.no_grad():
            features_list = self.backbone(dummy_input)
            last_features = features_list[-1]  # (B, C, H, W)
        feature_dim = last_features.shape[1]  # e.g., 768
        
        # CBAM for spatial and channel attention on feature maps
        self.cbam = CBAM(feature_dim, ratio=16)
        
        # Global attention on pooled features
    ...
    def forward(self, x):
        ...
        feat_map = features_list[-1]  # (B, C, H, W) spatial map
        
        # Apply CBAM to spatial feature map
        attended_map = self.cbam(feat_map)
        
        # Global average pooling
        pooled_features = F.adaptive_avg_pool2d(attended_map, (1, 1)).flatten(1)  # (B, C)
...
# Use the DINOv2-based model
model = DINOv2AttentionModel(num_classes=num_classes).to(DEVICE)
print("[INFO] DINOv2 Attention model with Adapted CBAM ready.")
...
torch.save(model.state_dict(), os.path.join(RESULTS,'dinov2_cbam_attention_model_final.pth'))
...
# 
# Note: For ViT, target the last attention layer for interpretability.
# -----------------------------
def find_target_layer(model):
    # For ViT, target the last attention layer in the backbone
    for name, module in reversed(list(model.backbone.named_modules())):
        if 'attn' in name and hasattr(module, 'out_proj'):
            return module
    # Fallback
    return model.backbone.norm  # Or last norm
